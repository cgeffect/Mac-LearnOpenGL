...
// ffplay -video_size 55x114 -pixel_format argb 1.rgb
int WriteRGBA(const std::string &filename,
             const Pixel32 *pxl,
             uint16 imageWidth,
             uint16 imageHeight) {
    int result = -1;
    ...
    if ((fp = fopen(filename.c_str(), "wb+")) != nullptr) {
        int64_t written = 0;
        written += fwrite((const char *)pxl, sizeof(unsigned char), (size_t)(imageWidth * imageHeight * 4), fp);
        fclose(fp);
        result = written != 0;
    }
    return result;
        ...
        if (FT_Load_Glyph(face, gindex, FT_LOAD_NO_BITMAP) == 0) {
            ...
            float advance = face->glyph->advance.x >> 6;
                        ...
                        for (Spans::iterator s = outlineSpans.begin(); s != outlineSpans.end(); ++s) {
                            for (int w = 0; w < s->width; ++w) {
                                pxl[(int)((imgHeight - 1 - (s->y - rect.ymin)) * imgWidth + s->x - rect.xmin + w)] =
                                Pixel32(255, 0, 0, 255); // mac 大端模式 a r g b
                                std::cout << "r: " << outlineCol.r << "g: " << outlineCol.g << "b: " << outlineCol.b << "a: " << s->coverage << std::endl;
                            }
                        }
                        // Then loop over the regular glyph spans and blend them into the image.
                        //然后循环遍历常规字形span并将它们混合到图像中。
...
//                        WriteTGA(fileName, pxl, imgWidth, imgHeight);
                        WriteRGBA(fileName, pxl, imgWidth, imgHeight);
                        delete[] pxl;
                    }
                }
            }
...
int main_freetype() {
    ...
    // Open up a font file.
    std::ifstream fontFile(font_name, std::ios::binary);
    if (fontFile) {
